    ...
    <div className='max-w-5xl w-full mx-auto space-y-8'>
        <div className='flex gap-4 items-center'>
            <div className='aspect-video flex-shrink-0 w-1/3 relative'>
                <Image src={product.imagePath} fill alt={product.name} layout="fixed" className='object-cover rounded-xl'/>
            </div>
            <div>
...
function Form({priceInCents,productId}:{priceInCents:number,productId:string}){
    ...
    async function handleSubmit(e:FormEvent){
      ...
      if(orderExists){
        SetErrorMessage("You have already purchased this product. Try downloading from My Orders page")
        setIsLoading(false)
        return
      }
    ...
    return <form onSubmit={handleSubmit}>
        <Card>
            <CardHeader>
                ...
                {errorMessage && <CardDescription className='text-descriptive'>{errorMessage}</CardDescription>}
            </CardHeader>
            <CardContent>
            <PaymentElement/>
            <div className='mt-4'>
